...
{
//#region vars
    var collisionMap:FlxSprite = new FlxSprite();
...
//#region room gfx
        ...
        collisionMap.loadGraphic("assets/images/rooms/town/collision.bmp");
        collisionMap.visible = true; // Make it invisible, as it’s only used for collision detection
        add(collisionMap);
...
//#endregion
        ...
        // Do not add it directly to the state yet
...
//#endregion
    ...
    override public function update(elapsed:Float):Void
    {
        ...
        targetCircle.x = target.x;
        targetCircle.y = target.y;
    ...
    function isWalkable(x:Float, y:Float):Bool {
        // Calculate the collision point (bottom-center of the player sprite)
        var collisionX = Std.int(player.x + player.origin.x);
        var collisionY = Std.int(player.y + player.origin.y);
    
        // Since origin might not be set to bottom-center, adjust accordingly
        // If origin is (player.width / 2, player.height), which is bottom-center:
        collisionX = Std.int(player.x + player.width / 2);
        collisionY = Std.int(player.y + player.height);
    
        // If collisionMap is at (0,0), no need to adjust for collisionMap.x and collisionMap.y
    
        // Ensure coordinates are within bounds
        if (collisionX < 0 || collisionY < 0 || collisionX >= collisionMap.width || collisionY >= collisionMap.height) {
            return false;
        }
    
        // Get the pixel color at the collision point
        var pixelColor = collisionMap.pixels.getPixel(collisionX, collisionY);
    
        // Output the pixel color for debugging
        trace('Collision at (' + collisionX + ', ' + collisionY + '): ' + StringTools.hex(pixelColor, 8));
    
        // Check if the pixel is black (non-walkable)
        // Mask out alpha channel in case of transparency
        var colorWithoutAlpha = pixelColor & 0x00FFFFFF;
        return colorWithoutAlpha != 0x000000; // Return true if the pixel is not black (walkable)
    }
    ...
    private function movePlayer(elapsed:Float):Void {
        ...
        if (distance <= 0) {
            // Player is already at the target
            isMoving = false;
            targetCircle.visible = false;
            player.animation.play(currentDirection); // Switch to idle animation
            return;
        }
        ...
        var nextX = player.x + direction.x * speed * elapsed;
        var nextY = player.y + direction.y * speed * elapsed;
        ...
        // Update the walking animation based on the movement direction
